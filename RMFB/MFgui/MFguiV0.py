    ...
    try:
      ...
      while True:
        dirName=ez.diropenbox(msg='Navigate to '+selected[0]+' directory',
                    title='Workspace',
                    default=os.path.dirname(r'H:\Documents\ArcGIS'))
        if dirName:
          break
      if not dirName:
        break
...
#
  ...
  justArgs    ={k: v for k, v in argHelp.items() if v[0] !='option'
                and k not in('nam','mod','ras','rasterName','bgdb','fgeo','ogeo','gdb','clpgdb','clpBox')}
  if optArgs['noArc']:
      wkspcArgs     ={k: v for k, v in argHelp.items() if v[0] !='option'
                    and k in('ras','rasterName','bgdb','fgeo','ogeo','clpBox')}
  else:
  ...
  #print('justArgs used to call guiArgVals',justArgs)
