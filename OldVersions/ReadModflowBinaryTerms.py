...
#
  ...
  namFileDat.append(header)
#  print ("NameFile: \t %s" % sourcefile)
  with open(sourcefile, 'r') as f:
    for line in f.readlines()[1:]:
      if (not line.startswith('#') ):
        if len(line) >1:
         namFileDat.append(line.split()[0:4])
  f.close()

  NAMdf = pd.DataFrame(namFileDat, columns = header)
  MFpkgEntry = NAMdf[NAMdf['Initials'] == pkgInitials]
  fileName = MFpkgEntry['filename']
