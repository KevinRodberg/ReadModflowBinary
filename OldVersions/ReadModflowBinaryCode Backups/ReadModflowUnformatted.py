...
import struct
import sys
import ModflowUnformatted2Rasters as mfr
...
def getOptions(model_choices):
    ...
    -res cellsize,--resample=cellsize  Resample=5 for 5x5; default is 10x10
    ...
    -t STRESS,    --stress=STRESS      Process  stress periods '1-12,218,288'
    `                                       or Omit for all stress periods
                                            or 0 for option testing
    -r RASTERS,   --rasters=RASTERS    Output layer '1,3-4,7'
                                            or   0 for no rasters
    ...
    Example arguments and brief description:
     ...
     -g ECFMper.gdb -loc SAN -r 1 -bin -s 138
        ^              ^      ^    ^     ^
        |              |      |    |     | Output
   ...
   Model file location /      `-Output  layer = 1
    ...
    parser.add_argument("-vecbcf", "--vectorbcf",
                      action="store_true",
                      dest="vectorbcf",
                      help="Process binary flow budgets for flow vectors")
    parser.add_argument("-res", "--resample",
                      ...
                      dest="resample",
                      default="1",
                      help="resample=5 for 5x5; default is no resampling or 1x1")
    ...
    parser.add_argument("-str","--stress",
                        ...
                        help="Create Rasters for         \
                            a single stress period or   \
                            for multiple stress periods provide multiple values '1-12,218,288'   \
                           -- Omit option [-str] for all layers \
                           -- Use s value of 0 for none (option testing)"
                        )
...
if options.heads:
    ...
    mfr.read_headfile(headsfile,disdf,options.layerStr,options.stressStr,llorigin,SR)
...
if options.conc:
    ...
    llorigin = modelOrigins[options.model]
    mfr.read_concfile(concfile,disdf,options.layerStr,options.stressStr,llorigin,SR)
...
if options.cbc:
    ...
    llorigin = modelOrigins[options.model]
    mfr.read_cbcfile(cbcfilename,disdf,options.layerStr,options.stressStr,llorigin,SR,options.terms)
...
if options.vector:
    ...
    mfr.read_cbcVectors(cbcfilename,disdf,options.layerStr,options.stressStr,llorigin,SR,VectorTerms,cellsize)
