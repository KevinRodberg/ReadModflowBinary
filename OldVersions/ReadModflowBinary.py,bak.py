...
#
  ...
  if 'IN_MEMORY' in rasName:
    ...
    print ("Raster: {} {}".format(optArgs['geodb'], rasName))
...
#
  ...
  #
    ...
    if budget == 'FLOW_FRONT_FACE':
      ...
      rasterdir = os.path.join(optArgs['geodb'],rasterdir)
      rastermag = os.path.join(optArgs['geodb'],rastermag)
      ...
      if csizeMultiplier > 1:
        ...
        rasMagResamp= os.path.join(optArgs['geodb'],os.path.basename(rasMagResamp))
        rasDirResamp=os.path.join(optArgs['geodb'],os.path.basename(rasDirResamp))
        ...
        arrowFeatureX = arcpy.env.workspace +"\\" +\
                os.path.basename(rasDirResamp)+"arw"
        inMemFCX = os.path.join(optArgs['geodb'],os.path.basename(arrowFeatureX))
        ...
        if modelClips[optArgs['model']] != (0,0,0,0):
          ...
          inMemRasDirXclp = os.path.join(optArgs['geodb'],os.path.basename(rasDirResamp))
          ...
          arcpy.RasterToPoint_conversion(in_raster=inMemRasDirXclp,
                          ...
                          raster_field="VALUE")
          inRasterListXclp = os.path.join(optArgs['geodb'],os.path.basename(rasMagResamp)+\
                             ' Magnitude')
        ...
        if 'clp' in rasterdir:
            ...
            arrowFeature = os.path.join(optArgs['geodb'],rastDir +"arw")
